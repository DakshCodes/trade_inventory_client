...
const P3Form = ({ rawMaterials, setShowP2Form, showP2Form, getData, selectedProcessID, MaterialTypes, getDataType, selectedProduct }) => {
    ...
    useEffect(() => {
        if (selectedProduct) {
            ...
            formRef?.current?.setFieldsValue(selectedProduct.stage[2] || selectedProduct.stage[1])
        }
    }, [selectedProduct])
    ...
    const onFinish = async (values) => {
        ...
        try {
            ...
            if (selectedProduct.stage[2]) {
                console.log(selectedProduct.stage[2])
                response = await EditProductOnceInitialised(selectedProduct._id, values, 2);
            }
            else {
